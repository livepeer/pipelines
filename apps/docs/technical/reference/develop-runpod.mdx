...
Stopping pods helps lower costs on RunPod. Pods must be stopped before making any port mapping changes
Install runpod-cli on your local machine by running the following command:
```bash
https://docs.runpod.io/runpodctl/install-runpodctl
```
