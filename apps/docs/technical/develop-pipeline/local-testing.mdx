...
```
<Info>
If you are using RunPod, you cannot use the default port of 8888 due to a conflict with jupyter-lab
</Info

We should now have the ComfyStream server running on port `8889`, with media port `5678` listening on UDP.  It's also using the same workspace and sharing the available nodes / models as the ComfyUI engine you were using. We will provide this endpoint to the ComfyStream UI when starting a live stream.
