...
The `--download-models` and `--build-engines` flags will download the required models and build TensorRT engines for optimized inference. This process may take some time. You will only need these flags on the first run of the container, or when adding additional models.
...
The `--server` flag will start ComfyUI, ComfyStream and UI automatically. They can be accessed at:
