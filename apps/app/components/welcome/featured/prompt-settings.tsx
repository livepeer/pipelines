...
interface SettingsMenuProps {
  ...
  function SettingsMenu({ 
    ...
    const sliderParams = pipeline?.prioritized_params
      ?.filter((param: any) => 
        param.widget === "slider" || 
          ...
          {hasSliderParams ? (
            <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-1 -mr-1">
              {sliderParams.map((param: any) => {
                ...
                const currentValueMatch = inputValue.match(new RegExp(`--${commandId}\\s+([\\d\\.]+)`));
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          if (currentValueMatch) {
                            ...
                            const newInput = inputValue.trim() + (inputValue.trim() ? ' ' : '') + `--${commandId} ${value}`;
