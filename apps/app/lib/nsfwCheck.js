...
exports.safePrompts = void 0;
exports.getRandomSafePrompt = getRandomSafePrompt;
exports.isPromptNSFW = isPromptNSFW;
...
const DEFAULT_SYSTEM_PROMPT = "You are a content moderation assistant. Your task is to determine if a StableDiffusion image generation prompt is trying to generate nudity, explicit content, or NSFW imagery. Respond with either 'true' if the prompt is attempting to generate such content, or 'false' if it appears safe. Then provide a brief explanation of your decision.";
// Get a random safe prompt
function getRandomSafePrompt() {
    const randomIndex = Math.floor(Math.random() * exports.safePrompts.length);
    return exports.safePrompts[randomIndex];
}
 ...
 */
async function isPromptNSFW(prompt) {
    try {
        const openai = getOpenAIClient();
        // Use environment variable for system prompt if available, otherwise use default
        const systemPrompt = process.env.NSFW_CHECK_SYSTEM_PROMPT || DEFAULT_SYSTEM_PROMPT;
