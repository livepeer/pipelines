...
const mixpanel_1 = require("@/lib/analytics/mixpanel");
const share_params_1 = require("@/app/api/streams/share-params");
// Cache for shared params data
let sharedParamsCache = {};
function getStoredIds() {
    if (typeof window === "undefined")
        return {};
    const distinctId = (0, mixpanel_1.handleDistinctId)();
    const sessionId = (0, mixpanel_1.handleSessionId)();
...
const getSharedParamsInfo = async () => {
    if (typeof window === "undefined")
        return {};
    ...
    if (sharedParam) {
        ...
        if (sharedParamsCache[sharedParam]) {
            ...
            try {
                const { data, error } = await (0, share_params_1.getSharedParamsAuthor)(sharedParam);
                if (!error && data) {
                    sharedInfo = {
...
const track = async (eventName, eventProperties, user) => {
    ...
    try {
        ...
        if (!response.ok) {
            try {
                ...
                console.error(`HTTP error! status: ${response.status}. Failed to read error body.`);
            }
