...
exports.resetProcessingFlag =
  exports.cleanupOldPrompts =
  exports.processNextPrompt =
  exports.addToPromptQueue =
  exports.getPromptState =
  exports.initializePromptState =
    void 0;
...
const addToPromptQueue = async (
  ...
  return await (0, __1.withDbClient)(async dbClient => {
    const countResult = await dbClient
      ...
      .from(prompt_queue_1.promptQueue)
      .where(
        (0, drizzle_orm_1.eq)(prompt_queue_1.promptQueue.processed, false),
      );
...
const processNextPrompt = async (highlightDuration = 10000) => {
  return await (0, __1.withDbClient)(async dbClient => {
    ...
    try {
      ...
      const displayedPrompts = [promptItem.text, ...stateData.displayedPrompts];
      ...
      await dbClient
        ...
        .where(
          (0, drizzle_orm_1.eq)(prompt_queue_1.promptQueue.id, promptItem.id),
        );
      const countResult = await dbClient
        .select({ count: (0, drizzle_orm_1.sql)`count(*)` })
        .from(prompt_queue_1.promptQueue)
...
const cleanupOldPrompts = async (maxAgeHours = 24) => {
  return await (0, __1.withDbClient)(async dbClient => {
    ...
    const result = await dbClient
      ...
      .where(
        (0, drizzle_orm_1.and)(
          (0, drizzle_orm_1.eq)(prompt_queue_1.promptQueue.processed, true),
          (0, drizzle_orm_1.lte)(
            prompt_queue_1.promptQueue.processedAt,
            cutoffDate,
          ),
        ),
      )
