...
exports.deleteClip = exports.retrieveClip = exports.storeClip = exports.initDB = void 0;
...
const initDB = () => {
    return new Promise((resolve, reject) => {
        ...
        request.onerror = event => {
            const reqError = event.target.error;
...
const ensureBlobType = (blob, defaultType) => {
    if (blob.type) {
        return blob;
    }
...
const storeClip = async (clipBlob, filename, thumbnailBlob, prompt, recordingMode) => {
    ...
    try {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = () => {
                console.log(`Clip data for ${filename} stored successfully in IndexedDB.`);
                localStorage.setItem("daydream_has_pending_clip", "true");
                localStorage.setItem("daydream_pending_clip_type", videoType);
                console.log("Set localStorage flags: daydream_has_pending_clip=true, daydream_pending_clip_type=" +
                    videoType);
                if (prompt) {
                    ...
                    console.log("Removed localStorage flag: daydream_pending_clip_prompt");
                }
                resolve();
            };
            request.onerror = () => {
                const reqError = request.error;
                console.error(`Error putting data into IndexedDB store for ${filename}:`, reqError);
                reject(reqError);
            };
            transaction.oncomplete = () => {
                console.log(`Store transaction completed for ${filename}. Closing DB connection.`);
                db.close();
            };
            transaction.onerror = event => {
                const transError = event.target.error;
...
exports.storeClip = storeClip;
const retrieveClip = async () => {
    ...
    try {
        ...
        if (hasPendingClip !== "true") {
            console.log("No pending clip flag found in localStorage. No clip to retrieve.");
            return null;
        }
        console.log("Pending clip flag found in localStorage. Proceeding with IndexedDB retrieval.");
        const db = await (0, exports.initDB)();
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = () => {
                ...
                if (!clipData || !clipData.clipBuffer || !clipData.thumbnailBuffer) {
                    console.warn("Retrieved data from IndexedDB is missing key properties (clipBuffer or thumbnailBuffer). Cleaning up localStorage.", clipData);
                ...
                const videoType = clipData.blobType ||
                    localStorage.getItem("daydream_pending_clip_type") ||
                    "video/mp4";
                const thumbType = clipData.thumbnailType || "image/jpeg";
                console.log(`Determined video type: ${videoType}, thumbnail type: ${thumbType}`);
                try {
                    console.log("Reconstructing Blobs from ArrayBuffers...");
                    const reconstructedClipBlob = new Blob([clipData.clipBuffer], {
                        type: videoType,
                    });
                    const reconstructedThumbnailBlob = new Blob([clipData.thumbnailBuffer], { type: thumbType });
                    console.log(`Clip data for ${clipData.filename} retrieved and Blobs reconstructed successfully.`);
                    resolve({
                    ...
                    console.error("Error reconstructing Blobs from ArrayBuffers:", reconstructionError);
                    localStorage.removeItem("daydream_has_pending_clip");
                    localStorage.removeItem("daydream_pending_clip_prompt");
                    localStorage.removeItem("daydream_pending_clip_type");
...
exports.retrieveClip = retrieveClip;
