...
async function handleRequest(req: NextRequest, method: string, streamkey: string) {
  try {
    ...
    const credentials = Buffer.from(`${gateway.userId}:${gateway.password}`).toString("base64");
...
export async function OPTIONS(req: NextRequest, { params }: { params: { streamkey: string } }) {
  return handleRequest(req, 'OPTIONS', params.streamkey);
}

export async function HEAD(req: NextRequest, { params }: { params: { streamkey: string } }) {
