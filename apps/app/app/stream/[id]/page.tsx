    ...
    const handleSaveStream = async () => {
        if (streamFormRef.current) {
            ...
            if (streamFormRef.current?.isFormValid()) {
                ...
                try {
                    const {data: savedStream, error} = await upsertStream(updatedStream, updatedStream.author);
                ...
                <>
                    ...
                    {authenticated ? (
                        <>
                            ...
                            <div className="pt-4">
                                <Button onClick={handleSaveStream} className="mt-4" disabled={isSubmitting}>
                                    {isSubmitting ? <>Saving <LoaderCircleIcon
                                        className="w-4 h-4 animate-spin"/></> : "Save"}
                                </Button>
                            </div>
                        </>
                    ) : (
