...
impl RedisClient {
    ...
    pub async fn add_prompt_to_queue(&self, prompt: Prompt) -> Result<usize> {
        ...
        let _: () = conn.lpush(&recent_key, &entry_json).await?;
        let _: () = conn.ltrim(&recent_key, 0, MAX_RECENT_PROMPTS - 1).await?;
    ...
    pub async fn get_recent_prompts(
        ...
        let entries_json: Vec<String> = conn.lrange(&recent_key, 0, limit as isize - 1).await?;
