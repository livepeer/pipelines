...
export class StreamApi {
  public static async applyPromptToStream(
    promptText: string,
    streamKey: string,
    gatewayHost: string,
    apiUser: string,
  ...
  private static async applyPromptAttempt(
    promptText: string,
    streamKey: string,
    gatewayHost: string,
    apiUser: string,
    ...
    const { cleanedPrompt, quality, creativity } =
      this.parsePromptParameters(promptText);
  ...
  private static parsePromptParameters(promptText: string): PromptParameters {
    let cleanedPrompt = promptText;
    let quality = 3.0;
    let creativity = 0.6;

    // Parse --quality parameter
    const qualityRegex = /--quality\s+(\d+(?:\.\d+)?)/;
    const qualityMatch = qualityRegex.exec(promptText);
    if (qualityMatch) {
      const parsedQuality = parseFloat(qualityMatch[1]);
      if (!isNaN(parsedQuality)) {
        quality = Math.max(1.0, Math.min(5.0, parsedQuality)); // Clamp between 1.0 and 5.0
        cleanedPrompt = cleanedPrompt.replace(qualityMatch[0], "");
      }
    }

    // Parse --creativity parameter
    const creativityRegex = /--creativity\s+(\d+(?:\.\d+)?)/;
    const creativityMatch = creativityRegex.exec(cleanedPrompt);
    if (creativityMatch) {
      const parsedCreativity = parseFloat(creativityMatch[1]);
      if (!isNaN(parsedCreativity)) {
        creativity = Math.max(0.1, Math.min(1.0, parsedCreativity)); // Clamp between 0.1 and 1.0
  ...
  private static buildComfyUIWorkflow(
    promptText: string,
    quality: number,
    creativity: number,
  ): any {
